    ...
    public partial class Form1 : Form
    {
        ...
        private void guna2GradientButton3_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < guna2DataGridView2.RowCount - 1; i++)
            {
                ...
                if (i == 0)
                {
                    // Hàng đầu tiên, giá trị wt là 0
                    WastingTime[i] = 0;
                }
                else
                {
                    // Tính giá trị wt theo công thức
                    WastingTime[i] = Math.Max(0, (ArrivalTime[i - 1] + BurstTime[i - 1] + WastingTime[i - 1]) - ArrivalTime[i]);
                    TurnAroundTime += WastingTime[i] + BurstTime[i];
                }
                // Cập nhật giá trị WastingTime (wt) vào cột mới
                guna2DataGridView2.Rows[i].Cells[3].Value = WastingTime[i];

                // Tính giá trị AverageWastingTime (awt) theo công thức
                int AverageWastingTime = WastingTime.Sum() / (guna2DataGridView2.RowCount - 1);
                guna2HtmlLabel1.Text = "Average Wasting Time: " + AverageWastingTime;
                // Tính giá trị TurnAroundTime (tat) theo công thức
               
            
                int AverageTurnAroundTime = TurnAroundTime  / (guna2DataGridView2.RowCount - 1);
